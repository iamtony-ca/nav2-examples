...
#include "static_smac_planner/smoother.hpp"
#include "ament_index_cpp/get_package_share_directory.hpp"

using namespace static_smac_planner;  // NOLINT
...
class SmootherWrapper : public static_smac_planner::Smoother
{
public:
  explicit SmootherWrapper(const SmootherParams & params)
  : static_smac_planner::Smoother(params)
  {}
...
TEST(SmootherTest, test_full_smoother)
{
  ...
  static_smac_planner::SmootherParams params;
  ...
  *costmapi = *costmap;
  ...
  std::unique_ptr<static_smac_planner::GridCollisionChecker> checker =
    std::make_unique<static_smac_planner::GridCollisionChecker>(costmap_ros, size_theta, node);
  ...
  for (int i = path.size() - 1; i >= 0; --i) {
    pose.pose = static_smac_planner::getWorldCoords(path[i].x, path[i].y, costmap);
    pose.pose.orientation = static_smac_planner::getWorldOrientation(path[i].theta);
