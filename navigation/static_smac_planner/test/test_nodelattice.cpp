...
{
  std::string pkg_share_dir = ament_index_cpp::get_package_share_directory("static_smac_planner");
  std::string filePath =
    pkg_share_dir +
  ...
  static_smac_planner::LatticeMetadata metaData;
  static_smac_planner::MotionPrimitive myPrimitive;
  static_smac_planner::MotionPose pose;

  json jsonMetaData = j["lattice_metadata"];
  json jsonPrimatives = j["primitives"];
  json jsonPose = jsonPrimatives[0]["poses"][0];

  static_smac_planner::fromJsonToMetaData(jsonMetaData, metaData);
  ...
  std::vector<static_smac_planner::MotionPrimitive> myPrimitives;
  for (unsigned int i = 0; i < jsonPrimatives.size(); ++i) {
    static_smac_planner::MotionPrimitive newPrimative;
    static_smac_planner::fromJsonToMotionPrimitive(jsonPrimatives[i], newPrimative);
...
TEST(NodeLatticeTest, test_node_lattice_neighbors_and_parsing)
{
  ...
  static_smac_planner::NodeLattice::initMotionModel(
    static_smac_planner::MotionModel::STATE_LATTICE, x, y, angle_quantization, info);

  static_smac_planner::NodeLattice aNode(0);
  unsigned int direction_change_index = 0;
  aNode.setPose(static_smac_planner::NodeHybrid::Coordinates(0, 0, 0));
  static_smac_planner::MotionPrimitivePtrs projections =
    static_smac_planner::NodeLattice::motion_table.getMotionPrimitives(
    &aNode,
  ...
  EXPECT_NEAR(
    static_smac_planner::NodeLattice::motion_table.getLatticeMetadata(
      filePath)
    .grid_resolution,
    0.05, 0.005);
}
...
TEST(NodeLatticeTest, test_node_lattice)
{
  ...
  // Check defaults
  static_smac_planner::NodeLattice aNode(0);
  static_smac_planner::NodeLattice testA(49);
  ...
  *costmap = *costmapA;
  ...
  static_smac_planner::NodeLattice testC(49);
  ...
  testC.setPose(static_smac_planner::NodeLattice::Coordinates(10.0, 5.0, 4));
...
TEST(NodeLatticeTest, test_get_neighbors)
{
  ...
  static_smac_planner::NodeLattice node(49);
  ...
  *costmap = *costmapA;
  ...
  std::function<bool(const uint64_t &,
    static_smac_planner::NodeLattice * &)> neighborGetter =
    [&, this](const uint64_t & index,
    static_smac_planner::NodeLattice * & neighbor_rtn) -> bool
    {
      // because we don't return a real object
      return false;
    };

  static_smac_planner::NodeLattice::NodeVector neighbors;
...
TEST(NodeLatticeTest, test_node_lattice_custom_footprint)
{
  ...
  *costmapi = *costmap;
  ...
  static_smac_planner::MotionPrimitivePtrs motion_primitives =
    static_smac_planner::NodeLattice::motion_table.getMotionPrimitives(&node, direction_change_index);
