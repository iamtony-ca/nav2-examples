  ...
  std::vector<std::tuple<float, float, float>> * expansions_log)
{
  ...
  if (!areInputsValid()) {
    RCLCPP_ERROR(rclcpp::get_logger("AStarAlgorithm"), "▶ ERROR: areInputsValid areInputsValid.");
    return false;
  }
  ...
  NodeGetter neighborGetter =
    ...
    {
      if (index >= max_index) {
        // RCLCPP_ERROR(rclcpp::get_logger("AStarAlgorithm"), "▶ ERROR: index >= max_index.");
        return false;
      }
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    if (iterations % _terminal_checking_interval == 0) {
      ...
      if (static_cast<double>(planning_duration.count()) >= _max_planning_time) {
        RCLCPP_ERROR(rclcpp::get_logger("AStarAlgorithm"), "▶ ERROR: static_cast<double>(planning_duration.count()) >= _max_planning_time");
        return false;
      }
    }
  ...
  RCLCPP_ERROR(rclcpp::get_logger("AStarAlgorithm"), "▶ ERROR: ERROR last line");
  return false;
}
