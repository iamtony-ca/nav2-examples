// check_pause_condition.cpp

#include "amr_bt_nodes/check_pause_condition.hpp"

namespace amr_bt_nodes
{

CheckPauseCondition::CheckPauseCondition(const std::string& name,
                                       const BT::NodeConfiguration& config)
: BT::ConditionNode(name, config)
{
  // 필수 입력: node
  if (!getInput("node", node_)) {
    throw BT::RuntimeError("[CheckPauseCondition] Missing required input [node]");
  }
  ...
  flag_sub_ = node_->create_subscription<std_msgs::msg::Bool>(
      flag_topic_, qos,
      std::bind(&CheckPauseCondition::flagCallback, this, std::placeholders::_1),
      sub_options);
  ...
  RCLCPP_INFO(node_->get_logger(),
              "[CheckPauseCondition] Subscribed to topic: %s", flag_topic_.c_str());
}

void CheckPauseCondition::flagCallback(const std_msgs::msg::Bool::SharedPtr msg)
{
  last_flag_.store(msg->data, std::memory_order_relaxed);
  RCLCPP_DEBUG(node_->get_logger(), "[CheckPauseCondition] flag=%s",
               msg->data ? "true" : "false");
}
...
{
  factory.registerNodeType<amr_bt_nodes::CheckPauseCondition>("CheckPauseCondition");
}
