...
// [추가] 단일 goal이 현재 위치와 각 축 0.04m 이내면(도착 직전), reference 계산/검증을 건너뛴다.
  constexpr double kArrivedTol = 0.04;  // m (축별 비교)
  skip_validation_ = (global_goals.size() == 1 &&
                      std::fabs(global_goals[0].pose.position.x - robot_pos.x) <= kArrivedTol &&
                      std::fabs(global_goals[0].pose.position.y - robot_pos.y) <= kArrivedTol);
  ...
  // [추가] 검증 생략 모드: static을 '즉시 성공(빈 경로)'으로 두어
  //        WAITING_FOR_STATIC 핸들러가 그대로 dynamic 경로만 계산하도록 흘려보낸다.
  if (skip_validation_) {
    static_done_ = true;
    static_error_code_ = 0;
    start_time_ = node_->now();
    RCLCPP_INFO(logger_, "[ComputeValidatedPathSyncAction] Single goal at current pose (<=%.2fm). Skipping reference & validation.", kArrivedTol);
...
BT::NodeStatus ComputeValidatedPathSyncAction::onRunning()
{
  ...
  switch (current_state_) {
    ...
    case PlanningState::WAITING_FOR_DYNAMIC:
    {
      ...
      // [추가] 검증 생략 모드: actual 경로를 그대로 통과시키고 성공 처리.
      if (skip_validation_) {
        ...
        RCLCPP_DEBUG(logger_, "[ComputeValidatedPathSyncAction] Validation skipped. Passing actual path through.");
