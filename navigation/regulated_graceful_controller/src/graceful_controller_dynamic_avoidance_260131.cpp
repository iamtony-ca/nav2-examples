  ...
  const std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  clock_ = node->get_clock(); // [추가] Clock 초기화
  ...
  // [추가] 동적 장애물 회피 파라미터 선언 및 로드
  nav2_util::declare_parameter_if_not_declared(
    node, plugin_name_ + ".collision_check_duration", rclcpp::ParameterValue(2.0)); // N초
  nav2_util::declare_parameter_if_not_declared(
    node, plugin_name_ + ".lookahead_extension_dist", rclcpp::ParameterValue(2.0)); // M미터

  node->get_parameter(plugin_name_ + ".collision_check_duration", collision_check_duration_);
  node->get_parameter(plugin_name_ + ".lookahead_extension_dist", lookahead_extension_dist_);

  // 상태 초기화
  last_collision_check_time_ = rclcpp::Time(0);
  is_extended_lookahead_ = false;
  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  // [추가] 현재 Lookahead 거리 설정 (확장 여부 반영)
  double current_max_lookahead = params_->max_lookahead;
  if (is_extended_lookahead_) {
      current_max_lookahead += lookahead_extension_dist_; // M미터 확장
  }

  bool collision_detected_during_search = false; // 이번 루프에서 충돌 때문에 스킵한 점이 있는지 여부
  ...
  for (int i = transformed_plan.poses.size() - 1; i >= 0; --i) {
    ...
    // [수정] 확장된 Lookahead 거리를 기준으로 검사
    if (dist_to_target > current_max_lookahead) { continue; }

    // Lookahead 안쪽인데도 확장된 상태에서 너무 가까운 점은 스킵 (안정성)
    if (dist_to_goal < params_->max_lookahead) {
       // Goal 근처 로직 (Orientation 등)
      if (params_->prefer_final_rotation) {
       ...
       // 너무 가까운 점은 스킵하고 break (뒤쪽 점은 더 가까우므로)
      break;
    }
    ...
    if (simulateTrajectory(target_pose, costmap_transform, local_plan, cmd_vel, reversing)) {
      ...
      // [추가] 동적 장애물 회피 상태 업데이트 로직
      // 성공적인 Trajectory를 찾았을 때 수행
      
      if (collision_detected_during_search) {
          // 더 멀리 있는 점들이 충돌(장애물) 때문에 선택되지 못함 -> 현재 막혀있는 상태일 가능성 높음
          
          // [Goal 근처면 확장하지 않음]
          // 남은 거리가 기본 Lookahead보다 짧으면, 장애물이 있어도 Lookahead를 늘리지 않음
          if (dist_to_goal <= params_->max_lookahead) {
             // Pass: 타이머 시작 안 함, 확장 안 함. 
             // 결과적으로 가까운 경로가 막혔으므로 정지하게 됨.
          }

          else {
            // Goal이 멀리 남았을 때만 회피 시도
            if (last_collision_check_time_.seconds() == 0.0) {
                last_collision_check_time_ = clock_->now();
            } else {
                // N초 이상 지속되었는지 확인
                auto duration = (clock_->now() - last_collision_check_time_).seconds();
                if (duration > collision_check_duration_) {
                    is_extended_lookahead_ = true; // Lookahead 확장 발동!
                    RCLCPP_WARN_THROTTLE(logger_, *clock_, 2000, 
                      "Dynamic Obstacle Detected (> %.1fs). Extending Lookahead by %.1fm.", 
                      collision_check_duration_, lookahead_extension_dist_);
                }
            }
          }
      } else {
          // 충돌 없이 가장 멀리 있는 점을 바로 찾음 (경로가 뚫림)
          // 단, 현재 확장 모드라면, 일반 범위(max_lookahead) 내에서 안전한 점을 찾았을 때만 복구
          if (dist_to_target <= params_->max_lookahead) {
              is_extended_lookahead_ = false;
              last_collision_check_time_ = rclcpp::Time(0); // 타이머 리셋
          }
      }
      ...
      // [추가] 이 점(target_pose)으로 가는 경로가 충돌함
      // 루프는 더 가까운 점을 찾으러 계속됨
      collision_detected_during_search = true;
    }
  }
