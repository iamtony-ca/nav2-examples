...
#include "regulated_mppi_controller/controller.hpp"
...
std::shared_ptr<regulated_mppi_controller::MPPIController> getDummyController(
  TNode node, TFBuffer tf_buffer,
  TCostMap costmap_ros)
{
  auto controller = std::make_shared<regulated_mppi_controller::MPPIController>();
  std::weak_ptr<rclcpp_lifecycle::LifecycleNode> weak_ptr_node{node};
