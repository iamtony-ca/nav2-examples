...
#include "reverse_smac_planner/node_lattice.hpp"
#include "reverse_smac_planner/a_star.hpp"
...
TEST(AStarTest, test_a_star_2d)
{
  ...
  reverse_smac_planner::AStarAlgorithm<reverse_smac_planner::Node2D> a_star(
    reverse_smac_planner::MotionModel::TWOD, info);
  ...
  *costmap = *costmapA;
  ...
  std::unique_ptr<reverse_smac_planner::GridCollisionChecker> checker =
    std::make_unique<reverse_smac_planner::GridCollisionChecker>(costmap_ros, 1, lnode);
  checker->setFootprint(nav2_costmap_2d::Footprint(), true, 0.0);
  a_star.setCollisionChecker(checker.get());
  a_star.setStart(20u, 20u, 0);
  a_star.setGoal(80u, 80u, 0);
  reverse_smac_planner::Node2D::CoordinateVector path;
  ...
  reverse_smac_planner::AStarAlgorithm<reverse_smac_planner::Node2D> a_star_2(
    reverse_smac_planner::MotionModel::TWOD, info);
  a_star_2.initialize(
    false, max_iterations, it_on_approach, terminal_checking_interval,
...
TEST(AStarTest, test_a_star_se2)
{
  ...
  reverse_smac_planner::AStarAlgorithm<reverse_smac_planner::NodeHybrid> a_star(
    reverse_smac_planner::MotionModel::DUBIN, info);
  int max_iterations = 10000;
  ...
  *costmap = *costmapA;
  ...
  std::unique_ptr<reverse_smac_planner::GridCollisionChecker> checker =
    std::make_unique<reverse_smac_planner::GridCollisionChecker>(costmap_ros, size_theta, lnode);
  checker->setFootprint(nav2_costmap_2d::Footprint(), true, 0.0);
  ...
  reverse_smac_planner::NodeHybrid::CoordinateVector path;
...
TEST(AStarTest, test_a_star_analytic_expansion)
{
  ...
  info.cost_penalty = 1.7;
  reverse_smac_planner::AStarAlgorithm<reverse_smac_planner::NodeHybrid> a_star(
    reverse_smac_planner::MotionModel::REEDS_SHEPP, info);
  int max_iterations = 10000;
  float tolerance = 10.0;
  int it_on_approach = 10;
  int terminal_checking_interval = 5000;
...
TEST(AStarTest, test_a_star_lattice)
{
  ...
  info.lattice_filepath =
    ament_index_cpp::get_package_share_directory("reverse_smac_planner") +
  ...
  reverse_smac_planner::AStarAlgorithm<reverse_smac_planner::NodeLattice> a_star(
    reverse_smac_planner::MotionModel::STATE_LATTICE, info);
  int max_iterations = 10000;
  float tolerance = 10.0;
  int it_on_approach = 10;
  int terminal_checking_interval = 5000;
  ...
  *costmap = *costmapA;
  ...
  reverse_smac_planner::NodeLattice::CoordinateVector path;
  EXPECT_TRUE(a_star.createPath(path, num_it, tolerance, dummy_cancel_checker));
...
{
  reverse_smac_planner::MotionModel mm = reverse_smac_planner::MotionModel::UNKNOWN;  // unknown
  EXPECT_EQ(reverse_smac_planner::toString(mm), std::string("Unknown"));
  mm = reverse_smac_planner::MotionModel::TWOD;  // 2d
  EXPECT_EQ(reverse_smac_planner::toString(mm), std::string("2D"));
  mm = reverse_smac_planner::MotionModel::DUBIN;  // dubin
  EXPECT_EQ(reverse_smac_planner::toString(mm), std::string("Dubin"));
  mm = reverse_smac_planner::MotionModel::REEDS_SHEPP;  // reeds-shepp
  EXPECT_EQ(reverse_smac_planner::toString(mm), std::string("Reeds-Shepp"));

  EXPECT_EQ(
    reverse_smac_planner::fromString(
      "2D"), reverse_smac_planner::MotionModel::TWOD);
  EXPECT_EQ(reverse_smac_planner::fromString("DUBIN"), reverse_smac_planner::MotionModel::DUBIN);
  EXPECT_EQ(
    reverse_smac_planner::fromString(
      "REEDS_SHEPP"), reverse_smac_planner::MotionModel::REEDS_SHEPP);
  EXPECT_EQ(reverse_smac_planner::fromString("NONE"), reverse_smac_planner::MotionModel::UNKNOWN);
}
