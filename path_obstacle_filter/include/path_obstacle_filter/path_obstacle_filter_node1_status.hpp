...
// ADDED: To subscribe to the /robot_status topic
...
private:
  ...
  // ADDED: New callback for robot status
  void robotStatusCallback(const std_msgs::msg::String::SharedPtr msg);

  // REMOVED: Callbacks for Nav2 action status are no longer needed
  ...
  // REMOVED: Helper for Nav2 action status is no longer needed
  ...
  // ADDED: Subscription for robot status
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr robot_status_sub_;
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr filtered_cloud_pub_;

  // REMOVED: Subscriptions for Nav2 action status are no longer needed
  ...
  // CHANGED: Replaced Nav2-specific flags with a single, general-purpose flag
  bool is_robot_driving_ = false;
