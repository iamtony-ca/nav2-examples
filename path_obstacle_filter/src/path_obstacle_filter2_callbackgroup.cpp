...
: Node("path_obstacle_filter", options)
{
  ...
  // ADDED: Create callback groups. They must be created before being used.
  // Use MutuallyExclusive to ensure callbacks within the same group don't execute in parallel.
  data_callback_group_ = this->create_callback_group(
    rclcpp::CallbackGroupType::MutuallyExclusive);
  timer_callback_group_ = this->create_callback_group(
    rclcpp::CallbackGroupType::MutuallyExclusive);

  // ADDED: Use a SubscriptionOptions object to assign subscriptions to a callback group.
  ...
  // CHANGED: Assign the timer to its own callback group.
  timer_ = this->create_wall_timer(
    std::chrono::milliseconds(100),
    std::bind(&PathObstacleFilterNode::process, this),
    timer_callback_group_);

  RCLCPP_INFO(this->get_logger(), "PathObstacleFilterNode initialized successfully with callback groups.");
}
