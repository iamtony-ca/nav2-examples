...
: Node("path_obstacle_filter", options)
{
  ...
  // ADDED: Subscribing to /robot_status to determine if the robot is driving.
  robot_status_sub_ = this->create_subscription<std_msgs::msg::String>(
    "/robot_status", rclcpp::SystemDefaultsQoS(),
    std::bind(&PathObstacleFilterNode::robotStatusCallback, this, std::placeholders::_1));

  // REMOVED: Subscriptions to Nav2 action statuses are no longer needed.
...
void PathObstacleFilterNode::getParameters()
{
  ...
  this->declare_parameter("enable_data_merging", true);
  ...
  enable_data_merging_ = this->get_parameter("enable_data_merging").as_bool();
}

// REMOVED: Nav2 action status callbacks and helpers are no longer needed.
// void PathObstacleFilterNode::updateGoalStatus(...)
// void PathObstacleFilterNode::navToPoseStatusCallback(...)
// void PathObstacleFilterNode::navThroughPosesStatusCallback(...)

// ADDED: New callback to handle robot status messages
void PathObstacleFilterNode::robotStatusCallback(const std_msgs::msg::String::SharedPtr msg)
{
  std::lock_guard<std::mutex> lock(data_mutex_);
  const std::string& status = msg->data;

  // Set the driving flag to true only for active navigation/planning states
  if (status == "DRIVING" || status == "PLANNING" || status == "FOLLOWING_WAYPOINTS") {
    if (!is_robot_driving_) {
      RCLCPP_INFO(this->get_logger(), "Robot status is '%s'. Activating path obstacle filter.", status.c_str());
    }
    is_robot_driving_ = true;
  } else {
    if (is_robot_driving_) {
      RCLCPP_INFO(this->get_logger(), "Robot status is '%s'. Deactivating path obstacle filter.", status.c_str());
    }
    is_robot_driving_ = false;
  }
}
...
{
  // 1. 로봇 주행 상태 확인: status_manager로부터 받은 상태를 확인.
  bool is_robot_driving;
  {
    std::lock_guard<std::mutex> lock(data_mutex_);
    is_robot_driving = is_robot_driving_;
  }
  
  if (!is_robot_driving) {
    // 주행 중이 아닐 때의 처리
    if (path_received_) {
      // RCLCPP_INFO(this->get_logger(), "Robot is not driving. Reverting to raw sensor data passthrough.");
      std::lock_guard<std::mutex> lock(data_mutex_);
  ...
  // 3. 스레드 안전하게 데이터 복사
  // REMOVED: Goal reached logic is now handled by the status_manager. This node only filters.
