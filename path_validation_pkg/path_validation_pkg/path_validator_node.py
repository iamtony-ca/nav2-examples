...
class PathValidator(Node):
    def __init__(self):
        ...
        # (추가) 시작점과 목표점을 발행할 퍼블리셔 생성
        self.start_pose_publisher = self.create_publisher(PoseStamped, '/validation_start_pose', 10)
        self.goal_pose_publisher = self.create_publisher(PoseStamped, '/validation_goal_pose', 10)
    ...
    def listener_callback(self, msg: PoseArray):
        ...
        # (추가) 시작점과 목표점 발행
        self.start_pose_publisher.publish(start_pose)
