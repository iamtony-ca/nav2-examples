...
class LaserDeskewNode(Node):
    def __init__(self):
        ...
        # 드라이버 stamp 규약 보정 (stamp가 끝 빔이면 True)
    ...
    def on_scan(self, scan):
        ...
        q_i = slerp(q0, q1, alpha)                       # (M,4)
        trans_i = (1.0 - alpha)[:, None] * p0 + alpha[:, None] * p1  # (M,3)
        p_odom = quat_rotate(q_i, p_laser) + trans_i     # R_i*p + t_i

        # 5) T_ref^-1 적용: p_out = R1^T (p_odom - p1) = quat_rotate(conj(q1), ...)
        q1_conj = np.array([-q1[0], -q1[1], -q1[2], q1[3]])
        p_out = quat_rotate(q1_conj, p_odom - p1)        # laser(t_ref) 기준

        # 6) PointCloud2 발행 (frame_id = 센서 프레임 유지!)
        header = Header()
        header.stamp = t_ref.to_msg()
        header.frame_id = laser_frame
        pts = np.column_stack([p_out.astype(np.float32), inten])  # (M,4)
