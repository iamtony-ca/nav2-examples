...
class FleetDecisionNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f"FleetDecisionNode Refactored. ID: {self.my_id}")
    ...
    def _reset_wait_and_run(self):
        if self._pre_moving_stop_type != MovingStopType.TYPE_NONE:
            self.get_logger().info("[Decision] Clear -> RUN")
