...
#include <memory>
#include <string>
#include "ammr_theta_star_planner/theta_star_planner.hpp"
#include "ammr_theta_star_planner/theta_star.hpp"

namespace ammr_theta_star_planner
{
...
{
  RCLCPP_INFO(logger_, "CleaningUp plugin %s of type ammr_theta_star_planner", name_.c_str());
  planner_.reset();
}

void ThetaStarPlanner::activate()
{
  RCLCPP_INFO(logger_, "Activating plugin %s of type ammr_theta_star_planner", name_.c_str());
...
{
  RCLCPP_INFO(logger_, "Deactivating plugin %s of type ammr_theta_star_planner", name_.c_str());
  auto node = parent_node_.lock();
  ...
  const double & dist_bw_points)
{
  ...
  geometry_msgs::msg::PoseStamped p1;
  // 생성된 pose들의 frame_id를 명시적으로 설정 (안전을 위해)
  ...
  for (unsigned int j = 0; j < raw_path.size() - 1; j++) {
    ...
    // [Modified] 방향(Yaw) 계산 로직 추가
    double dx = pt2.x - pt1.x;
    double dy = pt2.y - pt1.y;
    
    // 현재 구간(pt1 -> pt2)의 각도를 계산하여 쿼터니언으로 변환
    double theta = std::atan2(dy, dx);
    p1.pose.orientation = nav2_util::geometry_utils::orientationAroundZAxis(theta);
    ...
    int loops = static_cast<int>(distance / dist_bw_points);
    double sin_alpha = dy / distance;
    double cos_alpha = dx / distance;

    for (int k = 1; k < loops; k++) {
      p1.pose.position.x = pt1.x + k * dist_bw_points * cos_alpha;
      p1.pose.position.y = pt1.y + k * dist_bw_points * sin_alpha;
      // Orientation은 위에서 계산한 현재 구간의 방향(theta)을 그대로 유지
      pa.poses.push_back(p1);
    }
  }
...
PLUGINLIB_EXPORT_CLASS(ammr_theta_star_planner::ThetaStarPlanner, nav2_core::GlobalPlanner)
